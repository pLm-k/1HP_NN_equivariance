...
def measure_len_width_1K_isoline(data: Dict[str, "DataToVisualize"]):
    ...
    for index, key in enumerate(["t_true", "t_out"]):
        ...
        if datapoint.data.max() > T_gwf + 1:
            levels = [T_gwf + 1]
            CS = plt.contour(
                datapoint.data.T, levels=levels, cmap="Pastel1", extent=(0, 1280, 80, 0)
            )
...
def measure_loss(
    ...
    for case, dataloader in dataloaders.items():
        ...
        mae_closs = torch.Tensor(
            [
                0.0,
            ]
            * output_channels
        )
        rmse_closs = torch.Tensor(
            [
                0.0,
            ]
            * output_channels
        ...
        for x, y in dataloader:
            ...
            if rotate_inference:
                ...
                y_pred = (
                    model(
                        (
                            rt.safe_center_crop(x.cpu(), rt.get_safe_size(x.cpu()))
                            if crop
                            else x.cpu()
                        )
                        .to(device)
            ...
            start_pos = (
                (y.shape[2] - required_size[0]) // 2,
                (y.shape[3] - required_size[1]) // 2,
            )
            y = y[
                :,
                :,
                start_pos[0] : start_pos[0] + required_size[0],
                start_pos[1] : start_pos[1] + required_size[1],
            ]

            # normed losses
            for channel in range(y_pred.shape[1]):
                mse_loss[channel] += MSELoss(reduction="sum")(
                    y_pred[:, channel], y[:, channel]
                ).item()
            ...
            # losses in Celsius
            for channel in range(y_pred.shape[1]):
                mae_closs[channel] += L1Loss(reduction="sum")(
                    y_pred[:, channel], y[:, channel]
                ).item()
                rmse_closs[channel] += MSELoss(reduction="sum")(
                    y_pred[:, channel], y[:, channel]
                ).item()
                ...
                if vT_case == "temperature":
                    ...
                    pbt_closs[channel] += (
                        torch.sum(
                            torch.abs(y_pred[:, channel] - y[:, channel])
                            > pbt_threshold[channel]
                        )
                    ).item()
        ...
        if vT_case == "temperature":
            ...
            unit = "m/s"
        results[case] = {
            "MSE [-] per channel": output_mse,
            f"MAE [{unit}] per channel": output_mae,
            f"RMSE [{unit}] per channel": output_rmse,
        }

        if vT_case == "temperature":
            results[case]["PBT (percentage bigger than threshold 0.1C in [%]"] = (
                output_pbt
            )
    return results


def save_all_measurements(
    settings: SettingsTraining,
    len_dataset,
    times,
    solver: Solver = None,
    ...
    with open(
        ...
        / f"measurements_{settings.case}.yaml",
        "w",
    ) as f:
        ...
        f.write(
            f"duration of whole process including visualisation in seconds: {(times['time_end'] - times['time_begin'])}\n"
        )
        if settings.case in ["train", "finetune"]:
            f.write(
                f"duration of initializations in seconds: {times['time_initializations'] - times['time_begin']}\n"
            )
            f.write(
                f"duration of training in seconds: {times['time_training'] - times['time_initializations']}\n"
            )
        ...
        if settings.case in ["train", "finetune"]:
            f.write(
                f"best model found after epoch: {solver.best_model_params['epoch']}\n"
            )
            f.write(
                f"best model found with val loss: {solver.best_model_params['loss']}\n"
            )
            f.write(
                f"best model found with train loss: {solver.best_model_params['train loss']}\n"
            )
            f.write(
                f"best model found with val RMSE: {solver.best_model_params['val RMSE']}\n"
            )
            f.write(
                f"best model found with train RMSE: {solver.best_model_params['train RMSE']}\n"
            )
            f.write(
                f"best model found after training time in seconds: {solver.best_model_params['training time in sec']}\n"
            )
    ...
    if settings.case in ["train", "finetune"]:
        with open(
            Path.cwd()
            / "runs"
            / settings.destination.parent
            / f"measurements_all_runs.csv",
            "a",
        ) as f:
            # name, settings.epochs, epoch, val loss, train loss, val rmse, train rmse, train time
            f.write(
                f"{settings.destination.name},{settings.epochs},{solver.best_model_params['epoch']}, {round(solver.best_model_params['loss'], 6)}, {round(solver.best_model_params['train loss'], 6)}, {round(solver.best_model_params['val RMSE'], 6)}, {round(solver.best_model_params['train RMSE'], 6)}, {round(solver.best_model_params['training time in sec'], 6)}\n"
            )
