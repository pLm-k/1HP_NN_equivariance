...
from postprocessing.visualization import (
    ...
    infer_all_rotate_and_summed_pic,
)
...
def run(settings: SettingsTraining):
    ...
    if settings.problem == "2stages":
        if settings.use_ecnn:
            ...
            model = Cont_G_UNet(
                in_channels=input_channels, init_features=32, max_freq=4
            ).float()
        else:
    ...
    save_all_measurements(
        settings, len(dataloaders[which_dataset].dataset), times, solver
    )  # , errors)
    if settings.visualize:
        visualizations(
            ...
            settings.device,
            plot_path=settings.destination / f"plot_{which_dataset}",
        ...
        for angle in [0, 30, 45, 60, 90]:
            summed_dif_pic = infer_all_rotate_and_summed_pic(
                model,
                dataloaders[which_dataset],
                settings.device,
                rotate_inference=settings.rotate_inference,
            ...
            plot_avg_error_rotated_cellwise(
                dataloaders[which_dataset],
                summed_dif_pic,
                {"folder": settings.destination, "format": pic_format},
                angle=angle,
            )
            mean_error_strings.append(
                f"Mean Error (Equivariance {angle}): {summed_dif_pic.mean()}"
            )
...
def save_inference(model_name: str, in_channels: int, settings: SettingsTraining):
    ...
    for datapoint in (data_dir / "Inputs").iterdir():
        ...
        if settings.rotate_inference:
            y_out = rotate_and_infer(
                data.squeeze(0),
                [-1, 0],
                model,
                load_yaml(model_name, "info"),
                settings.device,
            ).to(settings.device)
        else:
